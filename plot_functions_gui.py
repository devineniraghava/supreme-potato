...
def plot_meas(t = 16, vmin=29, vmax=36):
    ...
    new = xr.open_dataset(
        "D:/PALM/campus_3_gui_24/OUTPUT/campus_3_gui_24_3d.000.nc", engine="netcdf4")
    ...
    plt.savefig('plots_GUI/GUI_plot_meas_{}.png'.format(t), format='png')
    plt.show()
    return a1
...
def plot_simulation(t = 16, vmin=29, vmax=36):
    ...
    a = new["theta"].isel(time=16, zu_3d=19).fillna(0)
    a.loc[:] = np.nan
    new_attributes = {'units': '°C', 'long_name': 'Temperature'}
    ...
    a1 = new["theta"].isel(time=t, zu_3d=19).fillna(0)
    
    b1 = new["im_t_indoor_mean"].isel(time=t, zw_3d=19).fillna(0)
    ...
    fig5.savefig('plots_GUI/GUI_plot_sim_{}.png'.format(t), format='png')
    plt.show()
    return a
...
def plot_diff(t = 16, vmin=29, vmax=36):
    ...
    plt.savefig('plots_GUI/GUI_diff_plot_{}.png'.format(t), format='png')
